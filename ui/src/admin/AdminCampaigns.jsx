  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCampaigns(campaigns.map(c => c._id === id ? { ...c, isDeleted: true } : c));
      } else toast.error(data.msg || "Archive failed");
                  ...
                  {campaigns.map(c => (
                    <tr key={c._id} className={"border-t hover:bg-gray-50" + (c.isDeleted ? " bg-gray-50" : "")}>
                      ...
                      <td className="p-3 text-center">
                        <span className={"font-medium " + (c.Status === "Active" ? "text-green-600" : "text-orange-500")}>{c.Status}</span>
                      </td>
